...
enum EntityType   { PLAYER, PLATFORM, MOVING_PLATFORM, NONE };
...
private:
    ...
    bool mMissionSuccess    = false;
    bool mMissionDecided    = false;
...
public:
    ...
    void update(float deltaTime, Entity *collidableEntities1, int collisionCheckCount1,
                                 Entity *collidableEntities2, int collisionCheckCount2);
    void render();
    ...
    void moveLeft()  { mAcceleration.x = -6.0f; mDirection = LEFT;  }
    void moveRight() { mAcceleration.x =  6.0f; mDirection = RIGHT; }
    ...
    //Moving platform's setters
    void setAsMovingPlatform(bool isMoving) 
        {mIsMovingPlatform = isMoving; 
         if (isMoving) mBaseY = mPosition.y; }
    void setMovementRange(float range) 
        { mMovementRange = range; }
    void setMoveSpeed(float speed) 
        { mMoveSpeed = speed; }
    bool isMovingPlatform() const 
        { return mIsMovingPlatform; }

    //fuel
    bool hasFuel() const { return mFuel > 0; }
    void consumeFuel(float amount) { 
        if (mFuel != 0.0f) {mFuel -= amount; }
        else {mFuel = 0.0f;}
    }
    float getFuel() const { return mFuel; }

    //missions
    bool isMissionDecided() const { return mMissionDecided; }
    bool isMissionSuccess() const { return mMissionSuccess; }
    void setMissionDecided(bool decision)  {  mMissionDecided = decision; }
    void setMissionSuccess(bool decision)  {  mMissionSuccess = decision; }
};
