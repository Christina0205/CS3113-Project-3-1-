    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (mVelocity.y > 0) 
            {
                ...
                if (!mMissionDecided) {
                    if (collidableEntity->mEntityType == MOVING_PLATFORM) {
                        mMissionDecided = true;
                        mMissionSuccess = false;   
                    }
                    else if (collidableEntity->mEntityType == PLATFORM) {
                        mMissionDecided = true;
...
void Entity::checkCollisionX(Entity *collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {            
            if (!mMissionDecided && (collidableEntity->mEntityType == MOVING_PLATFORM))
            {
                mMissionDecided = true;
                mMissionSuccess = false;
            }
            else if (!mMissionDecided && (collidableEntity->mEntityType == PLATFORM)) {
                mMissionDecided = true;
                mMissionSuccess = false;
            }
...
void Entity::update(float deltaTime, Entity *collidableEntities1, int collisionCheckCount1,
                                     Entity *collidableEntities2, int collisionCheckCount2)
{
    if(mEntityStatus == INACTIVE) return;
    if (mMissionDecided == true) return;

    resetColliderFlags();

    if (mEntityType == MOVING_PLATFORM)
    {
        float movementRange = 100.0f;
        float speed = 2.0f;
        float baseY = 600 - 2 * 100.0f;
        mPosition.y = baseY + sin(GetTime() * speed) * movementRange;
        return; 
    }
    ...
    checkCollisionY(collidableEntities1, collisionCheckCount1);
    checkCollisionX(collidableEntities1, collisionCheckCount1);
